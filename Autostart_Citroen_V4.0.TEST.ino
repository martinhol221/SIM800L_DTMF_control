...
int StTime = map(TempDS0, 50, -25, 1000, 6000);            // при -25 крутим стартером 6 сек, при +50 крутим 1 сек
    StTime = constrain(StTime, 1000, 6000);                // ограничиваем диапазон работы стартера от 1 до 6 сек
    Timer =  map(TempDS0, 50, -25, 30, 180);               // при -25 крутим прогреваем 30 мин, при +50 - 5 мин
    Timer =  constrain(StTime, 30, 180);                   // ограничиваем таймер в зачениях от 5 до 30 минут
    count = 1;                                             // переменная хранящая число попыток запуска
...
Serial.println ("out >");
 if (heating == true && count > 1) SMS_send = true;        // отправляем смс СРАЗУ только в случае незапуска c первой попытки
