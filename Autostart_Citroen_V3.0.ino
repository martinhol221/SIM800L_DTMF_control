...
#define ONE_WIRE_BUS 4 // и настраиваем  пин 9 как шину подключения датчиков DS18B20
...
#define ACTIV_Pin 10    // на светодиод и реле первого положения замка зажигания (для активации возбуждения генератора)
...
//#define Taho_Pin A1   // на преобразователь ШИМ-Аналог от тахометра
...
void setup() {
  ...
  SIM800.begin(9600);
  Serial.println("Starting SIM800L. 1.7 18/06/2017"); 
  SIM800_reset();
             }
...
void loop() {
  if (SIM800.available()) { // если что-то пришло от SIM800 в направлении Ардуино
    while (SIM800.available()) k = SIM800.read(), at += char(k),delay(1); // набиваем в переменную at
      ...
      delay(50), SIM800.println("ATH0"), Serial.println("Incoming call 2"), Timer = 60, enginestart();
...
//  } else if (at.indexOf("+DTMF: 0") > -1) {pin[poz]=0, poz++;
    } else if (at.indexOf("+DTMF: *") > -1) {pin[0]=0, pin[1]=0, pin[2]=0, poz=0;
               delay(100), SIM800.println("AT+VTS=\"9\""); 
    } else if (at.indexOf("+DTMF: #") > -1) {pin[poz]=1, poz++, Serial.println("#"), SMS_send = true;                     
    } else if (at.indexOf("+CMTI: \"SM\",") > -1) {Serial.println(at); 
               SIM800.println("AT+CMGR=1"), delay(20), SIM800.println("AT+CMGDA=\"DEL ALL\""), delay(20);  
...
//  Serial.print(" || Taho: "), Serial.print(analogRead(Taho_Pin));
...
void enginestart()   // цикл запуска двигателя
{
Serial.println ("enginestart() //////////"), SIM800.println("AT+VTS=\"7,7\"") ;
int count = 3;   // переменная хранящая число оставшихся потыток зауска
int StarterTime = 1000;  // переменная хранения время работы стартера (1 сек. для первой попытки)  
 while (Vbat > 10.00 && digitalRead(Feedback_Pin) == LOW && digitalRead(STOP_Pin) == LOW && count > 0) 
    ...
    digitalWrite(STARTER_Pin, LOW),  delay (7000); // отключаем реле, выжидаем 4 сек.
    Vbat =        analogRead(BAT_Pin), delay (100); // замеряем напряжение АКБ 3 раза
    Vbat = Vbat + analogRead(BAT_Pin), delay (100); // с интервалом в 0.3 сек. 
    Vbat = Vbat + analogRead(BAT_Pin), delay (100);
 ...
 //   Serial.print("Vbat^ = "), Serial.print(Vbat), Serial.print (" V. | count = "), Serial.println(count); 
    ...
    if (Vbat > 13.00) // если АКБ заряжается напряжением выше 13.00
//  if (analogRead(Taho_Pin) > 512) // если на ноге ардуино напряжение более напряжения питания то запуск удачен
        { 
        heating = true, Serial.println (" Vbat > 13.00,  heating = true , Engine started, break ");
        digitalWrite(ACTIV_Pin, HIGH), SIM800.println("AT+VTS=\"7,2,1,4,9\""), delay(3000);
        break; // считаем старт успешным, выхдим из цикла запуска двигателя
        }
    else // если статра нет вертимся в цикле
        { 
        heating = false, Serial.println (" Vbat < 13.00,  heating = false");
        digitalWrite(ON_Pin, LOW), digitalWrite(ACTIV_Pin, LOW);
        SIM800.println("AT+VTS=\"8,8,8\""), delay(3000);
        }
    }
SIM800.println("ATH0");
if (sms_report == true) SMS_send = true; // шлем смс всегда если настроено
