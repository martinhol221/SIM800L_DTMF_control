...
заменить #SI-M8-00-12-34-58#... на свои придуманные циферки 12-74-12-34-77-D2  (пример)  */
...
int Timer2 = 31080;      // часовой таймер (60 sec. x 60 min. / 10 = 360 )
...
void detection(){ // условия проверяемые каждые 10 сек  
    ...
    if (heating == true && Vbat < 11.0) Serial.println("Low voltage"), heatingstop(); 
     ...
     //  Автозапуск при понижении температуры ниже -18 гранусов, при -25 смс оповещение каждых 3 часа
    if (Timer2 == 2 && Temp0 < -18 && Temp0 != -127) Timer2 = 1080, Timer = 120, enginestart();  
    if (Timer2 == 1 && Temp0 < -25 && Temp0 != -127) Timer2 = 1080, SMS_send = true;    
...
int StarterTime = 1000;  // переменная хранения время работы стартера (1 сек. для первой попытки)  
 while (Vbat > 10.00 && digitalRead(Feedback_Pin) == LOW && digitalRead(STOP_Pin) == LOW && count > 0) 
    ...
    if (Vbat > 13.40) // если АКБ заряжается напряжением выше 13.40
        { 
        heating = true, Serial.println (" Vbat > 13.40,  heating = true , Engine started, break ");
        ...
        { 
        heating = false, Serial.println (" Vbat < 13.40,  heating = false");
...
if (heating == false) SMS_send = true;  // шлем смс только в случае незапуска
}
