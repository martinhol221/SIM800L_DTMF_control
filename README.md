...
# Возмонные проблемы и их устраниение:
...
* если после этого связи с модемом нет то проверьте какие логические уровни на ножках `TX` и `RX` модема, допустимый диапазон 2.5...3.1, при 3.3 тоже работать будет но это кощунство. [лог нормальнойработы](https://github.com/martinhol221/SIM800L_DTMF_control/blob/master/img/normal.log)
...
* если темпаратура с датчиков `TempDS0 - TempDS2` имеет значение 86, 87 или 88 град, то они физически не подключены, подключитеих

* если устройство включает стартер на рабртающем двигателе то не подключен провод обратной связи `FB` - подключите его

* если машина заводится и потом сама себе глошнет, то устройство не корректно замеряет напряжение заряда, необходима коллебровка. Если напряжение в мониторе порта не соответствует действительности то необходимо экспериментально подобрать `m = 65.....72;` пока напряжение на мультиметре и в мониторе порта не окажутся приблизительно одинаковыми.

* если температура с датчиков отображается; 86 - не подключен первый датчик, 87- не подключен второй датчик, 88 -не подключен третий датчик, проверьте подключение
