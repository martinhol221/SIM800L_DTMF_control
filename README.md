...
[Актуальный скетч 4.2 для загрузки через Arduino IDE](https://raw.githubusercontent.com/martinhol221/SIM800L_DTMF_control/master/Autostart_Citroen_V4.2.ino) в Arduino Pro Mini (8Mhz/3.3v), 

![](https://github.com/martinhol221/SIM800L_DTMF_control/blob/master/img/loading.JPG)

Последние ихменения в прошивке
...
2 Задаём время работы стартера и таймера обратного отсчета, в зависимости от показаний датчика температуры двигателя `Temp[0]`.
...
## Обходчик иммобилайзера:
  ...
  Обходчик представляет собой две катушки с равным количесвом витков, намотанные одним и тем же проводом, по верх антенны на замке зажигания и на ключ (чипе от ключа). Катушки соеденяются последовательно, свободные концы катушек соеденяются выводами `REL` и `NO`
реле, тем самым последнее, при включении замыкает контур ретранслируя сигнал от чипа на штатную антенну замка зажигания.
...
Определение местоположения модема по GPS-координатам, с использованием базовых станциий сотового оператора, точность при этом как в смартфоне без GPS от 100 до 800 м, в зависимосте от местности, в городе обычно 100-200 м.

* Для активации отправки ссылки с GPS-кординатой на карту гугл в СМС отчете, нужно раскоментировать строку:

`SIM800.print("\n https://www.google.com/maps/place/"), SIM800.print(LAT), SIM800.print(","), SIM800.print(LNG);`


* Для активации отправки GPS-кординат на народмон, в том числе для построения треков, нужно раскоментировать строки:

` SIM800.print("\n#LAT#"),          SIM800.print(LAT);           // Широта по геолокации `

` SIM800.print("\n#LNG#"),          SIM800.print(LNG);           // Долгота по геолокации `

Важно понимать, что если вы хотите испльзовать устройство как маяк для слежки за кем-либо то, изготавливать и продавать устройство с такой функцией то, в теории у вас могут быть проблемы с законом.
...
`https://www.google.com/maps/place/53.705334,27.220032` - ссылка на карту 

![](https://github.com/martinhol221/SIM800L_DTMF_control/blob/master/img/inSMS3.jpg)


## 6. Отправка показаний датчиков на сервер narodmon.ru
...
`#LAT#53.705334` - Широта  (опция)

`#LNG#27.220032` - Долгота (опция)
...
## 7. Прием команд из приложения Народмон 2017
...
## 8. Отключение зажигания при низком напряжении и превышении температуры выше 90 градусов
