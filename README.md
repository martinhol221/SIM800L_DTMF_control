...
* добавлено включение реле `OUT5` для запитки подогрева сидений, только при успешном старте, по общему таймеру прогрева

* первое SMS уходит если была хотябы отдна не успешная попытка запуска, вторая смс за 2 минуты до окончания прогрева
...
## 6. Отключение зажигания при низком напряжении
