...
* Адрес устройства на сервере народмон `MAC = "80-01-AA-00-00-00";` - нули нужно заменить на свои придуманные цифры 

* Адрес устройства на сервере народмон `SENS = "VasjaPupkin";` - аналогично
...
* вход `STOP_Pin A2` - на концевик педали тормоза (АКПП) или на датчик нейтральной передачи  в МКПП, на плате `IN2`

* вход `PSO_Pin A3`  - на датчик давления масла, кому покажется мало детектирования по напряжению (опция), на плате `IN3`

* вход `D2`- для датчиков объема или вибрации (аппаратное прерываение), на плате `IN0` (опция)

* вход `D3` - для подключения к датчику распредвала через оптопару, кому покажется мало детектирования по напряжению (опция)

* линия `L` - на пин 15 K-line шины в OBDII разъёме (опция)

* линия `K` - на пин 7 K-line шины в OBDII разъёме (опция)
